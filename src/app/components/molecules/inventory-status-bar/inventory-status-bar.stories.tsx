...
export const Skeleton: SkeletonStory = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Skeleton loader mirroring the InventoryStatusBar layout with three placeholder cards.'
      }
    }
  }
};
...
export const AccessibilityTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(section).toHaveAttribute('aria-labelledby', 'inventory-status-heading');
...
export const GridAccessibilityTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(grid).toBeInTheDocument();
    await expect(grid).toHaveAttribute('aria-label', `Inventory items (${mockItems.length} items)`);
  }
};
...
export const SemanticStructureTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    articles.forEach((article, idx) => {
      expect(article).toHaveAttribute('aria-labelledby', `inventory-item-${mockItems[idx].id}`);
    });
  }
};
...
export const EmptyState: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Displays an empty state message when no inventory items are provided.'
      }
    }
  }
};
...
export const EmptyStateTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(canvas.getByText('There are no inventory items being tracked at the moment')).toBeInTheDocument();
