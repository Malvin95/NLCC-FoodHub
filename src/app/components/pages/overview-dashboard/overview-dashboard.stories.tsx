...
const meta: Meta<typeof OverviewDashboard> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component: 'Overview dashboard composed of stat cards, trend chart, upcoming events, and inventory status sections.',
      },
    },
  },
  tags: ['autodocs'],
...
export const StructureTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    // Page heading
    await expect(canvas.getByRole('heading', { level: 1, name: 'Overview' })).toBeInTheDocument();
    await expect(canvas.getByText('A summary of key metrics and recent activities.')).toBeInTheDocument();

    // Stat cards
    const statTitles = ['Total Donations', 'Meals Distributed', 'Active Volunteers', 'Families Served'];
    ...
    const inventoryGrid = canvasElement.querySelector('[aria-label="Inventory items (4 items)"]');
...
export const LoadingStateTest: SkeletonStory = {
  ...
  play: async ({ canvasElement }) => {
    // Stat card skeletons
    const statSkeletons = canvasElement.querySelectorAll('[aria-label="Loading statistics"]');
    await expect(statSkeletons.length).toBe(4);

    // Chart skeleton
    await expect(canvasElement.querySelector('[aria-label="Loading chart data"]')).toBeInTheDocument();

    // Upcoming events skeleton
    await expect(canvasElement.querySelector('[aria-label="Loading upcoming events"]')).toBeInTheDocument();

    // Content feed skeleton
    await expect(canvasElement.querySelector('[aria-label="Loading content feed"]')).toBeInTheDocument();

    // Inventory skeleton
    await expect(canvasElement.querySelector('[aria-label="Loading inventory status"]')).toBeInTheDocument();
  },
};
