...
export const RendersUpcomingEvents: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Ensures the page heading, description, and upcoming events list render as expected.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByText(/Overview of upcoming and past events/i),
    ).toBeInTheDocument();
    ...
    const layoutGroup = canvas.getByRole("group", {
      name: /Events and calendar layout/i,
    });
    await expect(layoutGroup).toBeInTheDocument();

    const calendarRegion = canvas.getByRole("region", {
...
export const HasAccessibilityStructure: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Checks heading hierarchy and ARIA labeling for the upcoming events section.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const description = canvas.getByText(
      /Overview of upcoming and past events/i,
    );
...
export const LoadingSkeleton: SkeletonStory = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Skeleton state mirroring the events dashboard layout while data loads.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    const layoutGroup = canvas.getByRole("group", {
      name: /Events and calendar layout \(loading\)/i,
    });
    await expect(layoutGroup).toBeInTheDocument();
    ...
    const skeletonItems = canvas.getAllByRole("status", {
      name: /Loading event details/i,
      hidden: true,
    });
    await expect(skeletonItems.length).toBeGreaterThan(0);

    // Verify calendar skeleton is visible
    const calendarSkeleton = canvas.getByRole("region", {
      name: /Event Calendar \(loading\)/i,
    });
