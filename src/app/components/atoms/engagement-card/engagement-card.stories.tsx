...
const meta: Meta<typeof EngagementCard> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        component:
          "A simple engagement card displaying type, status, title, content, author, time, and response count.",
      },
    },
  },
  tags: ["autodocs"],
...
export const ClosedStatus: Story = {
  args: {
    ...
    content:
      "Thanks to all volunteers, sorting is complete. Further updates will be posted next week.",
    time: "1d ago",
    author: "Admin",
    responseCount: 12,
  },
...
export const RendersCoreFields: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Verifies all core content fields render correctly: type label, status, title, content, author, time, and response count.",
      },
    },
  },
  play: async ({ canvasElement }) => {
    ...
    await expect(
      canvas.getByText(/community pantry request/i),
    ).toBeInTheDocument();
    await expect(
      canvas.getByText(/we are organizing a food drive/i),
    ).toBeInTheDocument();
...
export const LoadingSkeleton: SkeletonStory = {
  ...
  parameters: {
    docs: {
      description: {
        story:
          "Skeleton loader that mirrors EngagementCard layout for graceful loading states.",
      },
    },
  },
};
