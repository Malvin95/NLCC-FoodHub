...
export const AccessibilityTest: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    await expect(article).toHaveAttribute('aria-labelledby', `inventory-item-${baseItem.id}`);

    // Heading with matching ID
    const heading = canvasElement.querySelector(`#inventory-item-${baseItem.id}`);
    await expect(heading).toBeInTheDocument();
    await expect(heading?.tagName).toBe('H3');

    // Status pill with aria-label
    const statusPill = canvasElement.querySelector(`[aria-label="Status: ${baseItem.status}"]`);
